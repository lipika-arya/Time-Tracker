...
def login_employer():
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            password = request.form['password']
            print("Employee ID:", employeeId)  # Debugging statement
            print("Password:", password)       # Debugging statement
            cursor.execute('SELECT password FROM employeeInfo WHERE employeeId=? AND (authorization="employer" OR employeeId=1)', (employeeId,))
            result = cursor.fetchone()  
            ...
            if result:
                ...
                message = "User does not exist or unauthorized access"
        cursor.close()
    except sqlite3.Error as error:
        print("Error while querying the database:", error)
        ...
        sqliteConnection.close()
    return render_template("login_employer.html", message=message)
...
def login_employee():
    ...
    try:
        ...
        if request.method == 'POST':
            employeeId = request.form['employeeId']
            password = request.form['password']
            cursor.execute('SELECT password FROM employeeInfo WHERE employeeId=? AND authorization="employee"', (employeeId,))
            result = cursor.fetchone()  
        ...
        sqliteConnection.close()
    return render_template("login_employee.html", message=message)
